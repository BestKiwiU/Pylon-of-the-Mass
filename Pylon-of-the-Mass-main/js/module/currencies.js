...
const RESETS = {
    energy_r: {
        unl: () => hasUpgrade("UNLOCK1"),
        pos: [0,21],
        desc: `重置能量、EEDs、充能能量以及A*和B*升级，以获得精炼能量 <b class='iconly-bolt-refined'></b>。根据能量获得更多精炼能量。`,
    ...
    energy_g: {
        unl: () => hasUpgrade("UNLOCK3"),
        pos: [20,41],
        desc: `重置精炼能量所做的一切，以及精炼能量、星星、挑战和C*、D*、E*升级，以获得银河能量 <b class='iconly-nova'></b>。根据精炼能量获得更多银河能量。`,
        get require() { return E(Number.MAX_VALUE) },
    ...
    meta: {
        unl: () => hasUpgrade("UNLOCK5"),
        pos: [40,41],
        desc: `重置银河能量所做的一切，以及银河能量、灵能精华、挑战和F*、G*升级，以获得元能量 <b class='iconly-infinity'></b>。`,
...
function createResetGridElement(id) {
    ...
    createGridElement('reset-'+id,{
        ...
        updateHTML() {
            ...
            ? `<p>${reset.desc}</p><p>获得 <b>+${format(tmp.currency_gain[id],0)}</b> ${curr.name} ${"require_dis" in reset ? `(下一个在 ${format(reset.require_dis(tmp.currency_gain[id].add(curr.amount)))})` : ""}</p>`
            : `达到 ${format(req, 0)} ${req_curr.name}`
